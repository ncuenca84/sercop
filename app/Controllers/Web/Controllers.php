...
// ══════════════════════════════════════════════════════════════════════════
// PLANTILLAS DOCX CONTROLLER
// ══════════════════════════════════════════════════════════════════════════
class PlantillasDocxController extends BaseController
{
    public function index(): void
    {
        $plantillas = DB::select(
            "SELECT * FROM plantillas_docx WHERE tenant_id = ? AND deleted_at IS NULL ORDER BY tipo ASC, nombre ASC",
            [tenantId()]
        );
        $this->view('plantillas_docx.index', compact('plantillas') + ['title' => 'Plantillas Word']);
    }
    ...
    public function store(): void
    {
        ...
        if (empty($_FILES['plantilla']['name'])) {
            View::flash('error', 'Seleccione un archivo .docx');
            $this->redirect('/plantillas-docx');
            return;
        }
        ...
        if ($ext !== 'docx') {
            View::flash('error', 'Solo se aceptan archivos .docx');
            $this->redirect('/plantillas-docx');
            return;
        }

        $dir  = tenantStoragePath('plantillas_docx');
        if (!is_dir($dir)) mkdir($dir, 0755, true);

        $filename = 'plantilla_' . uniqid() . '.docx';
        $dest     = $dir . '/' . $filename;
        ...
        DB::insert('plantillas_docx', [
            'tenant_id'       => tenantId(),
            'nombre'          => trim($this->input('nombre', $file['name'])),
            'descripcion'     => trim($this->input('descripcion', '')),
            'tipo'            => $this->input('tipo', 'proforma'),
            'ruta_archivo'    => $dest,
            'nombre_original' => $file['name'],
            'subido_por'      => authId(),
        ]);

        View::flash('success', 'Plantilla subida correctamente.');
        $this->redirect('/plantillas-docx');
    }
    ...
    public function destroy(string $id): void
    {
        verifyCsrf();
        DB::query(
            "UPDATE plantillas_docx SET deleted_at = NOW() WHERE id = ? AND tenant_id = ?",
            [(int)$id, tenantId()]
        );
        View::flash('success', 'Plantilla eliminada.');
        $this->redirect('/plantillas-docx');
    }

    public function marcadores(string $id): void
    {
    ...
    public function generar(string $id): void
    {
        ...
        $plantillas = DB::select(
            "SELECT * FROM plantillas_docx WHERE tenant_id = ? AND deleted_at IS NULL AND activa = 1 ORDER BY tipo ASC, nombre ASC",
            [tenantId()]
        );
        $this->view('plantillas_docx.generar', compact('proceso', 'plantillas') + ['title' => 'Generar Documento Word']);
    }

    public function generarPost(string $id): void
    {
        $proceso      = Proceso::conInstitucion((int)$id);
        $plantillaId  = (int)$this->input('plantilla_id', 0);

        if (!$proceso || !$plantillaId) {
            View::flash('error', 'Datos inválidos.');
            $this->redirect("/procesos/{$id}/generar-docx");
            return;
        }
        ...
        try {
            $vars       = \Services\DocxTemplateService::buildVars($proceso);
            $outputPath = \Services\DocxTemplateService::procesarDocx($plantilla['ruta_archivo'], $vars);

            // Guardar en expediente digital del proceso
            $categoria = $plantilla['tipo'] === 'proforma' ? 'proforma' : 'otro';
            $filename  = $plantilla['tipo'] . '_' . date('Ymd_His') . '.docx';
            $destDir   = tenantStoragePath("procesos/{$id}/{$categoria}");
            if (!is_dir($destDir)) mkdir($destDir, 0755, true);
            $dest      = $destDir . '/' . $filename;
            rename($outputPath, $dest);

            DocumentoProceso::create([
                'proceso_id'     => (int)$id,
                'categoria'      => $categoria,
                'nombre_archivo' => $filename,
                'nombre_original'=> $plantilla['nombre'] . '.docx',
                'ruta_storage'   => $dest,
            ...
            DB::query("UPDATE plantillas_docx SET usos = usos + 1 WHERE id = ?", [$plantillaId]);

            // Descargar directamente
            header('Content-Type: application/vnd.openxmlformats-officedocument.wordprocessingml.document');
            header('Content-Disposition: attachment; filename="' . $plantilla['nombre'] . '_' . $proceso['numero_proceso'] . '.docx"');
            header('Content-Length: ' . filesize($dest));
            readfile($dest);
            exit;

        } catch (\Throwable $e) {
            View::flash('error', 'Error al generar documento: ' . $e->getMessage());
            $this->redirect("/procesos/{$id}/generar-docx");
        }
    }
}
